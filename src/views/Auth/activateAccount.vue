<template>
  <container title="Réactivation du compte">
    <form class="d-flex justify-content-center flex-column p-2 bg-dark mt-5">
      ...
      <button class="btn btn-outline-danger w-100" @click.prevent="reactivateAccount">Continuer</button>
      <button class="btn btn-outline-success w-100 my-2" @click="deleteAccount">Supprimer le compte ?</button>
...
export default defineComponent({
  ...
  setup() {
    ...
    const reactivateAccount = () => {
      axios.get(apiUrl+'/reactivate-account?email='+user.value.email)
          .then(response =>{
            if(response.data.user === true){
              user.value.id = response.data.data.id;
              axios.post(apiUrl+'/reactivate-account', user.value)
                  .then(response => {
                    user.value = {}
                    msg.value = "";
    ...
    return {user,msg,reactivateAccount,deleteAccount,gologin}
  }
})
</script>